	...
	<div id="payment">
				...
				if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) {
					...
					foreach ( $available_gateways as $gateway ) {
						...
						<li class="payment_method_<?php echo $gateway->id; ?>">
                            <div class="radio">
                                <label for="payment_method_<?php echo $gateway->id; ?>" class="custom-radio-wrapper">
                            <span class="custom-radio-container">
                                <input id="payment_method_<?php echo $gateway->id; ?>" type="radio" class="input-radio" name="payment_method" value="<?php echo esc_attr( $gateway->id ); ?>" <?php checked( $gateway->chosen, true ); ?> data-order_button_text="<?php echo esc_attr( $gateway->order_button_text ); ?>" />
                                <span class="custom-radio-icon"></span>
                            </span>
                                    <span><?php echo $gateway->get_title(); ?> <?php echo $gateway->get_icon(); ?></span>
                                </label>
                            </div>
                            <?php
		...
		<div class="form-row">
			...
			<?php
				...
				echo apply_filters( 'woocommerce_pay_order_button_html', '<input type="submit" class="button alt btn btn-custom2 min-width" id="place_order" value="' . esc_attr( $pay_order_button_text ) . '" data-value="' . esc_attr( $pay_order_button_text ) . '" />' );
			?>
