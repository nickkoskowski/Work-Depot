...
function mango_current_header(){
	...
	$mango_header_hide = get_post_meta ( $header_hide_id, 'mango_header_hide', true ) ? get_post_meta ( $header_hide_id, 'mango_header_hide', true ) : '';

	
    if($mango_header_hide==1){
       return;
    }
	else{
	
    $id = mango_current_page_id();
    $header = get_post_meta ( $id, 'mango_page_header', true ) ? get_post_meta ( $id, 'mango_page_header', true ) : '';
    if(!$header){
...
function mango_current_footer(){
	...
	$mango_hide_bottom_footer = get_post_meta ( $footer_hide_id, 'mango_bottom_footer_widget_hide', true ) ? get_post_meta ( $footer_hide_id, 'mango_bottom_footer_widget_hide', true ) : '';

	
    if($mango_hide_bottom_footer==1){
       return;
    }
else{
	$id = mango_current_page_id();
    $footer = get_post_meta ( $id, 'mango_footer_type', true ) ? get_post_meta ( $id, 'mango_footer_type', true ) : '';
    if(!$footer){
